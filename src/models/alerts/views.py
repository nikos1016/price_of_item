...
# @requires_login # redirect the user to 'users.login' if session['email'] is None
def create_alert():
    if request.method == 'POST':
        name = request.form['name']
        url = request.form['url']
        price_limit = float(request.form['price_limit']) # it should be float not string
    ...
    # What happend if it's a GET request
