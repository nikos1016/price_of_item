        ...
        return requests.post(
            ...
            data={
                ...
                "text": "We've found a deal! ({}). To navigate to the alert, visit {}".format(
                    self.item.url, "http://".format(self._id))
            }
        )

    @classmethod
    def find_needing_update(cls, minutes_since_update=AlertConstants.ALERT_TIMEOUT):
        last_updated_limit = datetime.datetime.utcnow() - datetime.timedelta(minutes=minutes_since_update) #10min ago
        print(last_updated_limit)
        return [cls(**elem) for elem in Database.find(AlertConstants.COLLECTION,
                                                      {"last_checked":
                                                           {"$lte": last_updated_limit}, # gte = greater than equal to, $lte = less than equal to
                                                       "active": True # only update when alert is activating
                                                       })]
